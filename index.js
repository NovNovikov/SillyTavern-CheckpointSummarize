...
const CHECKPOINT_HEADER_LINE_RE = /^\s*\[Checkpoint\s+\d+\s+\|\s+messages\s+-?\d+\s*-\s*-?\d+\]\s*$/i;
...
const DEFAULT_STATE = {
  ...
  settings: {
    ...
    injectionPosition: INJECTION_POSITION_AFTER_WORLD_INFO,
    injectionDepth: 2,
...
function getBlocksRevision(state = null) {
  ...
  return Number(s?.runtime?.blocksRevision ?? 0);
}

function bumpBlocksRevision(state = null) {
  const s = state ?? getState();
  s.runtime ??= {};
  const current = Number(s.runtime.blocksRevision ?? 0);
  s.runtime.blocksRevision = Number.isFinite(current) ? current + 1 : 1;
}
...
function getWorldInfoResolutionReason() {
  return lastWorldInfoResolutionReason || "unknown";
}

function setExtensionStatusError(message) {
  lastExtensionStatusError = String(message ?? "").trim();
}

function clearExtensionStatusError() {
  lastExtensionStatusError = "";
}

function setAutoModeRangeDebugInfo(text) {
...
function setAutoModeUiLock(locked) {
  if (locked && !autoModeUiLocked) {
    try {
      ...
      console.warn(`[${MODULE_NAME}] Failed to deactivate send buttons`, error);
    }
  ...
  if (!locked && autoModeUiLocked) {
    try {
      ...
      console.warn(`[${MODULE_NAME}] Failed to activate send buttons`, error);
    }
...
async function runAutoMode() {
  ...
  if (hasVisibleText(state.draft.summary ?? "")) {
    if (state.settings.autoApproveEnabled) {
      const chatNow = getChatMessages();
      ...
      try {
        ...
        if (lockOk) {
          ...
          if (hasVisibleText(postLockState.draft.summary ?? "")) {
            setExtensionStatusError("auto-approve lock failed");
          }
        }
      } catch (error) {
  ...
  if (!firstGap) {
    setAutoModeRangeDebugInfo("no gaps");
    return;
  }
  ...
  try {
    ...
    while (safetyCycles < maxCycles) {
      ...
      const loopGap = getFirstGapInfo(loopChat.length);
      if (!loopGap) break;
      const loopStartIndex = Number(loopGap.start);
...
function getChronologicalLockedBlocks() {
  const locked = getLockedBlocks();
  const ranges = locked
    .filter((b) => isRangeCheckpoint(b))
    .sort((a, b) => Number(a.startIndex) - Number(b.startIndex) || Number(a.endIndex) - Number(b.endIndex));
  const memoryOnly = locked
    .filter((b) => !isRangeCheckpoint(b))
    .sort((a, b) => Number(a.createdAt || 0) - Number(b.createdAt || 0));
  return [...ranges, ...memoryOnly];
}

function getCoverageGaps(chatLength) {
  ...
  for (const block of ranges) {
    ...
    const end = Math.min(total - 1, Number(block.endIndex));
...
function getLastLockedBlock() {
  const locked = getSortedRangeCheckpoints();
...
function getDraftRange(draft) {
  const start = parseDraftIndex(draft?.startIndex);
  const end = parseDraftIndex(draft?.endIndex);
  if (start === null || end === null) return null;
  if (start < 0 || end < start) return null;
  return { start, end };
}

function isDraftRangeInsideGap(draft, gapStart, gapEnd) {
...
function isZeroZeroRange(range) {
  return !!range && range.start === 0 && range.end === 0;
}

function isZeroZeroPlaceholder(range, gapStart, gapEnd) {
  if (!range) return false;
  return isZeroZeroRange(range) && (gapEnd - gapStart) >= 1;
}
...
function normalizeSummaryOutput(value) {
  ...
  text = text.replace(/[\u200B-\u200D\u2060\uFEFF]/g, "");
  // Strip injected checkpoint wrapper lines if model copied them into summary text.
  let lines = text.split(/\r?\n/);
  while (lines.length && CHECKPOINT_HEADER_LINE_RE.test(String(lines[0] ?? ""))) {
    lines.shift();
  }
...
function getNextStartIndex() {
  const chat = getChatMessages();
  const gap = getFirstGapInfo(chat.length);
  if (!gap) return chat.length;
...
function getInjectableLockedBlocks() {
  return getChronologicalLockedBlocks().filter((b) => b.inject !== false);
}
...
function collectWorldInfoLikeStrings(root, maxDepth = 4) {
  ...
  const walk = (value, depth, keyHint = "") => {
    ...
    for (const [k, v] of Object.entries(value)) {
      const key = String(k || "");
      if (denyKeyPattern.test(key) && !keyPattern.test(key)) continue;
      walk(v, depth + 1, key);
    }
  };
...
function formatChatLineForWIScan(message, includeNames) {
  const text = getMessageText(message);
  if (!includeNames) return text;
  const role = getMessageRole(message);
  const fallbackName = role === "user" ? "User" : "Assistant";
  const name = String(message?.name ?? "").trim() || fallbackName;
  return `${name}: ${text}`;
}

function buildChatForWIScanFromCurrentChat() {
  const ctx = getContext();
  const chat = Array.isArray(ctx?.chat) ? ctx.chat : getChatMessages();
  const includeNames = !!world_info_include_names;
  const lines = Array.isArray(chat)
    ? chat.map((msg) => formatChatLineForWIScan(msg, includeNames))
    : [];

  const hasUserMessage = Array.isArray(chat) && chat.some((msg) => getMessageRole(msg) === "user");
  if (!hasUserMessage) {
    const userName = String(ctx?.name1 ?? "User").trim() || "User";
    lines.push(includeNames ? `${userName}:` : "");
  }
...
function getLatestWorldInfoTextForDraft() {
  ...
  const deep = collectWorldInfoLikeStrings(latest, 4);
  if (deep.length) return deep[0];
  return "";
}

async function getLatestWorldInfoTextForDraftAsync() {
  setWorldInfoResolutionReason("start");
  const direct = getLatestWorldInfoTextForDraft();
  if (direct) {
    setWorldInfoResolutionReason("resolved from latest itemized prompt entry");
    return direct;
  }
  let latestIndex = -1;
  let latestMesId = -1;
  if (!Array.isArray(itemizedPrompts) || !itemizedPrompts.length) {
    setWorldInfoResolutionReason("itemizedPrompts is empty for current chat");
  } else {
    ...
    if (latestIndex < 0 || latestMesId < 0) {
      setWorldInfoResolutionReason("itemizedPrompts has no valid mesId entry");
    } else {
      try {
        const params = await itemizedParams(itemizedPrompts, latestIndex, latestMesId);
        if (!params || typeof params !== "object") {
          setWorldInfoResolutionReason("itemizedParams returned empty/non-object");
        } else {
          const paramFields = [
          ...
          for (const field of paramFields) {
            const normalized = normalizeWorldInfoText(field);
            if (normalized) {
              setWorldInfoResolutionReason("resolved from itemizedParams world info field");
              return normalized;
            }
          }

          const deepParams = collectWorldInfoLikeStrings(params, 5);
          if (deepParams.length) {
            setWorldInfoResolutionReason("resolved from deep itemizedParams scan");
            return deepParams[0];
          }
          setWorldInfoResolutionReason("itemizedParams present, but contains no readable world info text");
        }
      } catch (error) {
        console.warn(`[${MODULE_NAME}] Failed reading World Info snapshot from itemized params`, error);
        setWorldInfoResolutionReason(`itemizedParams error: ${String(error?.message ?? error ?? "unknown")}`);
      }
    }
  }

  // Final fallback: ask ST WI engine directly (no new message required).
  try {
    const ctx = getContext();
    const chatForWI = buildChatForWIScanFromCurrentChat();
    const maxContext = Number(getMaxContextSize() || 0);
    if (!Array.isArray(chatForWI) || !chatForWI.length) {
      setWorldInfoResolutionReason("WI engine fallback skipped: chatForWI is empty");
      return "";
    }
    if (!(maxContext > 0)) {
      setWorldInfoResolutionReason(`WI engine fallback skipped: maxContext is invalid (${String(maxContext)})`);
      return "";
    }
    if (Array.isArray(chatForWI) && chatForWI.length && maxContext > 0) {
      const globalScanData = {
        trigger: "normal",
      };
      const attempts = [true, false];
      const emptyAttempts = [];
      for (const dryRun of attempts) {
        ...
        if (fromEngine) {
          setWorldInfoResolutionReason(`resolved from WI engine fallback (dryRun=${String(dryRun)})`);
          return fromEngine;
        }
        emptyAttempts.push(`dryRun=${String(dryRun)}`);
      }
      setWorldInfoResolutionReason(`WI engine fallback returned empty (${emptyAttempts.join(", ")})`);
    }
  } catch (error) {
    console.warn(`[${MODULE_NAME}] Failed reading World Info from WI engine fallback`, error);
    setWorldInfoResolutionReason(`WI engine fallback error: ${String(error?.message ?? error ?? "unknown")}`);
  }

  if (!getWorldInfoResolutionReason() || getWorldInfoResolutionReason() === "start") {
    setWorldInfoResolutionReason("all WI resolution paths returned empty");
  }
  return "";
}

async function buildRawPromptWithOptionalWorldInfo(prompt, includeWorldInfo) {
  const basePrompt = String(prompt ?? "");
  if (!includeWorldInfo) return basePrompt;

  const worldInfo = await getLatestWorldInfoTextForDraftAsync();
  if (!worldInfo) return "";
  ...
  try {
    ...
    const summary = await runWithSelectedConnectionProfile(async () => {
      if (!state.settings.useProfilePromptStack) {
          const rawPrompt = await buildRawPromptWithOptionalWorldInfo(prompt, !!state.settings.useWorldbookInDraft);
...
async function lockDraftCheckpoint(options = {}) {
  ...
  const chat = getChatMessages();
  const draftRange = getDraftRange(state.draft);
  const summary = normalizeSummaryOutput(state.draft.summary ?? "");

  if (!summary) {
    if (!silent) toastr.warning("Draft summary is empty.", MODULE_NAME);
    setExtensionStatusError("draft summary is empty");
    return false;
  }

  if (!draftRange || draftRange.end >= chat.length) {
    if (!silent) toastr.warning("Invalid draft range.", MODULE_NAME);
  ...
  if (!rangeMessages.length) {
    if (!silent) toastr.warning("Cannot lock empty range.", MODULE_NAME);
    setExtensionStatusError("cannot lock empty range");
    return false;
  }
...
const SUMMARY_COMPRESSION_PRESETS = Object.freeze({
  huge: { ratio: 1 / 70, label: "Huge compression", formula: "1/70" },
  normal: { ratio: 1 / 50, label: "Normal compression", formula: "1/50" },
  light: { ratio: 1 / 35, label: "Light compression", formula: "1/35" },
  endless: { ratio: 1 / 20, label: "Endless VRAM", formula: "1/20" },
});
const DEFAULT_SUMMARY_COMPRESSION_PRESET = "normal";
...
function sanitizeSummaryCompressionPreset(value) {
  ...
  return Object.prototype.hasOwnProperty.call(SUMMARY_COMPRESSION_PRESETS, key)
    ? key
    : DEFAULT_SUMMARY_COMPRESSION_PRESET;
}

function getSummaryCompressionRatio(state = null) {
  const s = state ?? getState();
  const preset = sanitizeSummaryCompressionPreset(s?.settings?.summaryCompressionPreset);
  return SUMMARY_COMPRESSION_PRESETS[preset]?.ratio ?? SUMMARY_COMPRESSION_PRESETS[DEFAULT_SUMMARY_COMPRESSION_PRESET].ratio;
}

function getSummaryCompressionDisplay(state = null) {
  const s = state ?? getState();
  const preset = sanitizeSummaryCompressionPreset(s?.settings?.summaryCompressionPreset);
  return SUMMARY_COMPRESSION_PRESETS[preset] ?? SUMMARY_COMPRESSION_PRESETS[DEFAULT_SUMMARY_COMPRESSION_PRESET];
}
...
function applyNoBrainPreset(state) {
  state.enabled = true;
  state.injectionEnabled = true;
  state.settings.injectionPosition = INJECTION_POSITION_AFTER_WORLD_INFO;
...
function applyNoBrainUiLock() {
  ...
  const allowIds = new Set([
    ...
    "stcs-injection-template",
  ]);
  const allowButtonsInNoBrain = new Set([
    "stcs-export-checkpoints",
  ...
  const setControlHidden = (element, hidden) => {
    ...
    const wrapper = element instanceof HTMLButtonElement
      ? element
      : (element.closest(".checkbox_label, .stcs-connection-profile-row, label") ?? element);
    if (!(wrapper instanceof HTMLElement)) return;
    if (hidden) {
      if (!Object.prototype.hasOwnProperty.call(wrapper.dataset, "stcsPrevDisplay")) {
        wrapper.dataset.stcsPrevDisplay = wrapper.style.display ?? "";
      }
      wrapper.style.display = "none";
    } else if (Object.prototype.hasOwnProperty.call(wrapper.dataset, "stcsPrevDisplay")) {
      wrapper.style.display = wrapper.dataset.stcsPrevDisplay || "";
      delete wrapper.dataset.stcsPrevDisplay;
    }
  };
  ...
  controls.forEach((el) => {
    ...
    if (el instanceof HTMLButtonElement) {
      ...
      if (id === "stcs-import-checkpoints" || id === "stcs-import-current-summary") {
        el.disabled = generationActive;
        setControlHidden(el, generationActive);
      } else {
        el.disabled = false;
    ...
    if (!id) return;
    const shouldHideAndDisable = noBrainOn && !allowIds.has(id);
...
function applyTargetsFromRawBlock(rawBlockTokens) {
  ...
  const rawSummaryEstimate = Math.max(
    SUMMARY_MIN,
    Math.round(computedRawBlockTokens * getSummaryCompressionRatio(state)),
  );
...
function updateDraftRangeFromInputs() {
  ...
  const rawStart = parseDraftIndex(startInput.value);
  const rawEnd = parseDraftIndex(endInput.value);
  const start = rawStart === null ? null : clampNumber(rawStart, 0, chat.length - 1, null);
  const end = rawEnd === null ? null : clampNumber(rawEnd, 0, chat.length - 1, null);
...
function renderStatus() {
  ...
  if (!autoModeInFlight && !draftGenerationInFlight && autoModeUiLocked) {
    // Self-heal stuck lock state after interrupted/failed auto attempts.
    setAutoModeUiLock(false);
  }
  ...
  const summaryCompressionInlineRowEl = document.getElementById("stcs-summary-compression-inline-row");
  const summaryCompressionPresetInlineEl = document.getElementById("stcs-summary-compression-preset-inline");
  ...
  const unsummarizedCount = gaps.reduce((sum, g) => sum + (Number(g.end) - Number(g.start) + 1), 0);
  const unsummarizedTokensTotal = gaps.reduce(
  ...
  const compressionPreset = sanitizeSummaryCompressionPreset(state.settings.summaryCompressionPreset);
  if (summaryCompressionPresetInlineEl) summaryCompressionPresetInlineEl.value = compressionPreset;
  if (summaryCompressionPresetSubmenuEl) summaryCompressionPresetSubmenuEl.value = compressionPreset;
  if (summaryCompressionInlineRowEl instanceof HTMLElement) {
    summaryCompressionInlineRowEl.style.display = state.settings.noBrainModeEnabled ? "" : "none";
  }
  ...
  const calcResultText = (() => {
    ...
    const backendContext = Math.max(0, Number(calc.maxContext || 0));
    const availableContext = Math.max(0, Number(calc.availableTokens || 0));
  ...
  if (calcResultSubmenuEl) {
    calcResultSubmenuEl.textContent = calcResultText;
    calcResultSubmenuEl.style.display = state.settings.noBrainModeEnabled ? "none" : "";
  }
  if (calcResultExternalEl) {
    calcResultExternalEl.textContent = calcResultText;
...
function bindUiEvents() {
  ...
  const onSummaryCompressionPresetChange = (value) => {
    const state = getState();
    state.settings.summaryCompressionPreset = sanitizeSummaryCompressionPreset(value);
    applyTargetsFromRawBlock(Number(state.settings.targetRawBlockTokens || DEFAULT_STATE.settings.targetRawBlockTokens));
    saveState();
    renderStatus();
    if (state.settings.noBrainModeEnabled) {
      void runNoBrainMaintenanceCycle({ silent: true });
  ...
  summaryCompressionPresetInlineEl?.addEventListener("change", () => {
    onSummaryCompressionPresetChange(summaryCompressionPresetInlineEl.value);
  });
  summaryCompressionPresetSubmenuEl?.addEventListener("change", () => {
    onSummaryCompressionPresetChange(summaryCompressionPresetSubmenuEl.value);
  });
